...
<script>
    ...
    let permisos = JSON.parse('<?php echo json_encode($permisos) ?>');
        $(document).ready(() => {
            ...
            $('#listado').jtable({
               ...
               defaultSorting: 'Orde date ASC', // ordenado ascendente
                ...
                actions: {
                   listAction: '<?php echo site_url(); ?>registrar/docentes/lista',
                ...
                recordsLoaded: (event, data) => {
                    ...
                    $('.edit-row').click(function(e) {
                        ...
                        console.log(id),
                        //window.location.href = `<?php echo site_url(); ?>administrador/docentes/edit/${id}`;
                        //LoadRecordsButton.click();
  
                        $.ajax({
                                url: base_url + "registrar/docentes/edit",
                                method: "POST",
                                data:{id:id},
                                dataType: "json"
                        })
                            ...
                            // console.log(result);
                                ...
                                $(".modal-title-titulo").text("Editar Docente");        
     ...
     /*               $('.view-row').click(function(e) {
			           ...
			           // console.log(id);
                        $.ajax({
                            url: base_url + "registrar/docentes/view/" + id,
                            type:"POST",
                            success:function(resp){
                                $("#modal-docentes .modal-body").html(resp);
                                $("#modal-docentes").modal('show');
                                //alert(resp);
                            }
                        });
                    });
*/
                    $('.view-row').click(function(e) {
                        ...
                        $.ajax({
                            ...
                            // console.log(result);
                                ...
                                $(".modal-header-color").css("color", "white");
                                $(".modal-title-titulo").text("Datos del Docente");         
                                $("#modalCRUD").modal("show");      
                            })
                            .fail(function(jqXHR, textStatus, errorThrown) {
                            });
                    ...
                    $('.delete-row').click(function(e) {
                        ...
                        Swal.fire({
                            ...
                            if (result.value) {
                                console.log(id)
                                const url = `<?php echo site_url(); ?>registrar/docentes/delete/${id}`;
                                fetch(url)
                                    .then(res=>res.json())
                                    .then(res => {
                                        console.log(res)
...
const newRecord = () => {
    $("#form_docentes").trigger("reset");
    $(".modal-header-color").css("background-color", "#007bff");
    $(".modal-header-color").css("color", "white");
    $(".modal-title-titulo").text("Nuevo Docente");            
    $("#modalCRUD").modal("show");        
    id="";
    }

$("#form_docentes").submit(function(e){
   e.preventDefault();  
   // console.log(email);
    id =$.trim($("#id").val()); 
    dni = $.trim($("#dni").val());    
    ...
    $.ajax({
        url: "<?php echo base_url(); ?>registrar/docentes/store",
        type: "POST",
        dataType: "json",
